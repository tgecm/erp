...
export default function App() {
  ...
  // If not logged in, show ONLY the login page — nothing else
  if (!user) {
    return <LoginModal />;
  }

  // If superadmin, show the dedicated SuperAdmin Control Panel
  if (user.isSuperAdmin) {
    return <SuperAdminDashboard />;
  }

  return (
    <div className="h-screen w-screen max-w-full overflow-hidden bg-slate-100 dark:bg-black text-slate-800 dark:text-white flex flex-col font-sans antialiased transition-colors duration-200">
      {/* Top Header Navbar */}
      <Navbar />

      {/* Main Layout: Sidebar pinned to Left + Main Workspace Area */}
      <div className="flex flex-1 w-full overflow-hidden min-w-0">
        {/* Left Sidebar (Desktop) + Slide Drawer (Mobile) */}
        <Sidebar />

        {/* Main Content Area with Smooth Page Transition */}
        <main className="flex-1 p-2.5 sm:p-6 pb-24 md:pb-6 overflow-y-auto overflow-x-hidden bg-slate-100/70 dark:bg-black h-full w-full max-w-full min-w-0">
